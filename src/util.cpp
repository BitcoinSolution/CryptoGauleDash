...
const char * const BITCOIN_CONF_FILENAME = "cryptogaule.conf";
const char * const BITCOIN_PID_FILENAME = "cryptogauled.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "cryptogaule" is a composite category enabling all cryptogaule-related debug output
            if(ptrCategory->count(string("cryptogaule"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\cryptogauleCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\cryptogauleCore
    // Mac: ~/Library/Application Support/cryptogauleCore
    // Unix: ~/.cryptogaulecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "cryptogauleCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/cryptogauleCore";
#else
    // Unix
    return pathRet / ".cryptogaulecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty cryptogaule.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override cryptogaule.conf
